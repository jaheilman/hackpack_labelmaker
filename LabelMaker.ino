...
#define INIT_MSG   " GIMME A SECOND " // Text to display on startup
#define INIT_MSG2  "   TO WAKE UP   " // Text to display on startup
#define MODE_NAME  "AARON LABELMAKER" //these are variables for the text which is displayed in different menus. 
#define START_MSG  "   Please stop  "
...
void setup() {
  ...
  lcd.setCursor(0, 1);
  lcd.print(INIT_MSG2);  // print start up message
  ...
  yStepper.setSpeed(8);  // set first stepper speed (these should stay the same)
  xStepper.setSpeed(8);  // set second stepper speed (^ weird stuff happens when you push it too fast)
...
void loop() {
  ...
  switch (currentState) {  //state machine that determines what to do with the input controls based on what mode the device is in
    ...
    case Editing:  //in the editing mode, joystick directional input adds and removes characters from the string, while up and down changes characters
      ...
      lcd.print(text);
      lcd.setCursor(0, 1);
      lcd.print(text.substring(15)); // print char 16 to end
      ...
      if (text.length() < 15) {
        ...
        lcd.setCursor(text.length()-15, 1);
      }
      ...
      if (joyUp) {  //UP (previous character)
        ...
        else if (currentCharacter == 0) { //wrap
          currentCharacter = alphabetSize - 1;
        }
        delay(SCROLL_DELAY);  // Delay to prevent rapid scrolling
        ...
        else if (currentCharacter == (alphabetSize - 1)) { //wrap
          currentCharacter = 0;
        }
        delay(SCROLL_DELAY);  // Delay to prevent rapid scrolling
      } else {
      ...
      if (joyLeft) {
        ...
        if (text.length() > 0) {
          ...
          // lcd.setCursor(0, 1);
          // lcd.print(CLEAR);  //clear and reprint the string so characters dont hang
          // // lcd.setCursor(0, 0);
          // // lcd.print(MENU_CLEAR);  //clear and reprint the string so characters dont hang
          // // lcd.setCursor(1, 0);
          // // lcd.print(text);
          ...
          // // lcd.print(text.substring(16)); // need to print the second half of the text to line 2
        }
        delay(SCROLL_DELAY);  // Delay to prevent rapid multiple presses
